...
reject_matches \
  ...
  'SecItem(Add|CopyMatching|Delete|Update)|URLSession(\.shared|[[:space:]]*\()|SystemHTTPTransport\.make|KeychainCredentialClient\.system|keychain:[[:space:]]*\.system|LaunchAtLoginService\.backend|UNUserNotificationCenter\.current|SystemLoginLauncher\.open' \
  Tests App/UITests App/UITestSupport App/Benchmarks --glob '*.swift'
reject_matches \
...
reject_matches \
  ...
  '\b(Process|LibprocProcessScanner)[[:space:]]*\(' Tests App/UITests App/UITestSupport App/Benchmarks --glob '*.swift'
reject_matches \
...
reject_matches \
  "local window subclasses must use the presentation-rejecting fixture" \
  ':[[:space:]]*(NSWindow|NSPanel)\b' \
  Tests --glob '*.swift' --glob '!**/Native/**' --glob '!NonpresentingWindow.swift'
...
if mutation_files="$(rg -l "$global_mutation" Tests App/UITests App/UITestSupport App/Benchmarks --glob '*.swift')"; then
