...
Scripts/with-render-checks.sh 16 ${TOKEN_MENU_BAR_RENDER_CAPTURE_ONLY:+--capture-only} swift test "${build_args[@]}" --jobs "${TOKEN_MENU_BAR_BUILD_JOBS:-2}" --enable-code-coverage --no-parallel \
...
live_calls="$(
  xcrun llvm-cov export "$binary" -instr-profile "$profdata" | jq '
    [.data[].functions[] | select(.count > 0) |
      select((.filenames[0] | test("/(SystemKeychain|SystemHTTPTransport|PersistentDefaults|ApplicationActivation|SystemLoginLauncher)\\.swift$")) or
...
glue=(
  ...
  Sources/TokenMenuBarUI/Adapters/SystemLoginLauncher.swift
...
if ((missed > 0)); then
  ...
  while IFS= read -r file; do
    ...
    xcrun llvm-cov export "$binary" -instr-profile "$profdata" "$file" |
      jq -r --arg file "$file" '
        .data[].functions[] | select(.count == 0 and .filenames[0] == $file) |
        "\(.filenames[0]):\(.regions[0][0]): unexecuted \(.name)"
