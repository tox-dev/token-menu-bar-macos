...
rg -q '^Rendered-text assertions: 0; failures: 0$' "$probe/empty-output"
...
rg -q 'Expected at least 1 rendered-text assertions; found 0.' "$probe/required"
...
BASH
rg -q 'require the CI Rendered text job' "$probe/success"
...
rg -q 'Expected at least 1 rendered-text captures; found 0.' "$probe/missing"
status=0
"${capture[@]}" Scripts/with-render-checks.sh 1 --capture-only \
...
BASH
  ...
  rg -q 'exited without a completed, nonempty passing suite' "$probe/$outcome-suite"
done
