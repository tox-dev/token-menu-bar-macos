...
final class TooltipPanel: NSPanel, TooltipPanelPresenting {
  ...
  func show(
    ...
    let textBounds = label.cell!.cellSize(
      forBounds: CGRect(x: 0, y: 0, width: maximumTextWidth, height: .greatestFiniteMagnitude))
    let textSize = CGSize(
    ...
    // Attaching a child window orders it immediately, so its first frame must already be final.
