...
public final class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  public func applicationDidFinishLaunching(_ notification: Notification) {
    if controller.dependencies.presentsWindows { NSApplication.shared.setActivationPolicy(.accessory) }
    controller.start()
  }
...
public final class DeferredAppDelegate: NSObject, NSApplicationDelegate {
  ...
  private let persistsStatusItemPosition: Bool
  ...
  public func applicationDidFinishLaunching(_ notification: Notification) {
    if presentsWindows { NSApplication.shared.setActivationPolicy(.accessory) }
    showStatusShell()
  ...
  private func showStatusShell() {
    ...
    if persistsStatusItemPosition {
      item.autosaveName = StatusItemController.autosaveName(bundleIdentifier: Bundle.main.bundleIdentifier)
    }
    item.button?.title = "…"
...
public enum AppRunner {
  ...
  public static func bootstrap(
    ...
    applyVerificationAppearance(paths.verificationProfile?.appearance)
  ...
  public static func bootstrapDeferred(
    ...
    return DeferredAppDelegate(
      presentsWindows: presentsWindows, persistsStatusItemPosition: paths.verificationProfile == nil
    ) {
      var dependencies = try await LiveDependencies.makeDeferred(
        appInfo: appInfo, paths: paths, defaults: defaults, notificationCenter: notificationCenter, updater: updater,
        isSandboxed: isSandboxed, transport: transport, keychain: keychain, launchAtLogin: launchAtLogin)
  ...
  @MainActor private static func applyVerificationAppearance(_ appearance: VerificationProfile.Appearance?) {
