...
public final class StatusItemController {
  ...
  public var popoverVisible = false {
    didSet {
      ...
      if popoverVisible {
        ...
        restart(trigger: "popover-close", expanding: false)
      }
    }
  }
  ...
  public init(
    ...
    item = existingItem ?? statusBar.statusItem(withLength: 22)
    item.length = 22
    item.autosaveName = autosaveName
    item.button?.target = self
    item.button?.action = #selector(buttonClicked(_:))
    item.button?.sendAction(on: [.leftMouseDown, .rightMouseDown])
    // Wider settings edits must not wrap attachments and double the frozen status item's height.
    item.button?.cell?.usesSingleLineMode = true
    item.button?.cell?.wraps = false
  ...
  func layoutChanged(forgetting: Bool, trigger: String = "layout-change") {
    ...
    if trigger == NSWorkspace.didActivateApplicationNotification.rawValue, planner.context == layoutContext() {
      recordStatus(action: .deferred, trigger: trigger)
      return
    }
  ...
  func restart(trigger: String = "restart", expanding: Bool = true) {
    guard !popoverVisible else {
      recordStatus(action: .deferred, trigger: trigger)
      return
    }
    ...
    let index = expanding ? planner.begin(context: context, ladderCount: count) : planner.resume(ladderCount: count)
    apply(self.ladder[min(index, self.ladder.count - 1)])
  ...
  public func checkFit() -> Bool {
    ...
    if detailedLoggingEnabled {
      probe()
      log.logDebug(
        "status fit windowFrame=\(String(describing: item.button?.window?.frame)) "
          + "buttonFrame=\(String(describing: buttonFrameOnScreen)) "
          + "visibleRect=\(String(describing: item.button?.visibleRect))")
    }
    let fits = fits()
    let oldIndex = planner.index
    let context = layoutContext()
  ...
  @discardableResult
  public func restoreVisibility() -> Bool {
  ...
  public func handleClick(_ event: NSEvent?) {
    if Self.isSecondaryClick(event), let menu = menuProvider?() { return show(menu) }
    onClick?()
  }

  static func isSecondaryClick(_ event: NSEvent?) -> Bool {
    guard let event else { return false }
    return event.type == .rightMouseDown || event.type == .rightMouseUp || event.modifierFlags.contains(.control)
  }
