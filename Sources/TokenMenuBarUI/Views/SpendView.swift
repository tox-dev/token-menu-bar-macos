...
public struct SpendView: View {
  ...
  public var body: some View {
    VStack(alignment: .leading, spacing: 5) {
      ...
      .accessibilityValue(valueAppearance.accessibilityValue(presentation.summary))
      if let percent = presentation.spend.percent {
        UsageBar(
          percent: percent,
          color: valueAppearance == .stale ? Color(.tertiary) : Color(UsageColor.color(percent: percent)), label: title)
      }
