  ...
  private var metricPicker: some View {
    HStack(spacing: 8) {
      ...
      Picker(
        ...
        ForEach(
          HistoryMetricGroup.allCases.filter { group in presenter.availableMetrics.contains { $0.group == group } },
          id: \.self
        ) { group in
          Section(group.rawValue) {
            ForEach(presenter.availableMetrics.filter { $0.group == group }) { metric in
