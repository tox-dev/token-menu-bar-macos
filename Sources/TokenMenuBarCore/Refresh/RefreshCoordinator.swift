...
public final class RefreshCoordinator {
    ...
    await withTaskGroup(
      ...
      for await (id, result, credentialStatus, includedAnalytics, retryInterval, duration) in group {
        ...
        let applied = await apply(
          ...
          now: now)
        guard !Task.isCancelled, registryGeneration == self.registryGeneration else { continue }
        if let providerState = applied.providerState {
          state.applyProviderStates([id: providerState])
          rebuildStatus(now: clock.now())
        }
        if !applied.events.isEmpty { notify(applied.events) }
  ...
  private func apply(
    ...
    return ProviderApplyResult(
      providerState: next,
      events: paceNotifiedOnce(
        signInNotifiedOnce(events, for: id, credential: credentialStatus.state), snapshot: next.snapshot),
