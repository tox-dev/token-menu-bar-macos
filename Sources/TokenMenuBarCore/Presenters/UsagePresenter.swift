...
public struct WindowRow: Sendable, Hashable, Identifiable {
  ...
  public init(
    ...
    isSelected: Bool = true, display: UsageDisplay = .used
  ) {
    self.key = key
    self.window = window
    self.pace = pace
  ...
  public var percentText: String {
    Format.percent(used: window.usedPercent, display: display)
  }
  ...
  public func accessibilityValue(at now: Date) -> String {
    ...
    return
      "\(percentText) \(display.suffix)\(inactive)\(selection), \(resetText(at: now).lowercased()), "
      + pace.comparison(now: now)
  }
}
...
public struct ProviderCard: Sendable, Hashable, Identifiable {
  ...
  public var valueAppearance: UsageValueAppearance {
    UsageValueAppearance(availability: availability, hasSnapshot: fetchedAt != nil)
  }
...
public enum UsagePresenter {
  ...
  public static func cards(
    state: [ProviderID: ProviderState], enabled: Set<ProviderID>, samples: [WindowKey: [UsageSample]],
    options: UsageDisplayOptions = .standard, now: Date
  ) -> [ProviderCard] {
    state.keys.sorted().filter { isVisible(state[$0]!, enabled: enabled.contains($0)) }.map { provider in
      card(provider: provider, state: state[provider]!, samples: samples, options: options, now: now)
    }
  }
  ...
  public static func presentation(
    ...
    iconTone: StatusIconTone, isRefreshing: Bool, options: UsageDisplayOptions = .standard, now: Date
  ) -> UsagePresentation {
    let cards = state.keys.sorted().filter { isVisible(state[$0]!, enabled: enabled.contains($0)) }.map { provider in
      card(
        ...
        analytics: analytics[provider] ?? UsageAnalyticsPresentation(), options: options, now: now)
    }
  ...
  public static func card(
    provider: ProviderID, state: ProviderState, samples: [WindowKey: [UsageSample]],
    options: UsageDisplayOptions = .standard, now: Date
  ) -> ProviderCard {
    card(
      provider: provider, state: state, samples: samples, selected: nil,
      analytics: analyticsPresentation(state.analytics, now: now), options: options, now: now)
  }

  static func card(
    provider: ProviderID, state: ProviderState, samples: [WindowKey: [UsageSample]], selected: Set<WindowKey>?,
    analytics: UsageAnalyticsPresentation, options: UsageDisplayOptions, now: Date
  ) -> ProviderCard {
    ...
    let rows = (snapshot?.windows ?? []).filter { window in
      ...
      let pace = PaceEstimate.estimate(
        window: window, samples: samples[key] ?? [], workdays: options.paceWorkdays, now: now)
      return WindowRow(
        key: key,
        window: window,
        pace: pace,
        countdown: Format.countdown(to: window.resetsAt, now: now),
        resetClock: Format.resetClock(window.resetsAt, precision: window.resetPrecision, now: now),
    ...
    return ProviderCard(
      ...
      identity: options.hidePersonalInformation ? identity?.hidingPersonalInformation : identity,
      chips: chips(provider: provider, snapshot: snapshot, hidePersonalInformation: options.hidePersonalInformation),
      rows: rows,
  ...
  public static func spendPresentation(
    ...
    if let allowance = spend.creditAllowance {
      if let remaining = allowance.remaining {
        metrics.append(
          UsageMetricPresentation(
            title: "Remaining", value: CreditAllowance.formatted(remaining),
            help: "Provider-reported monthly credits, not currency."))
      }
      if let source = allowance.source {
        metrics.append(
          UsageMetricPresentation(
            title: "Allowance source", value: Format.humanize(source), help: "Who sets this credit allowance."))
      }
    }
    ...
    if spend.canPurchaseCredits {
      metrics.append(
        UsageMetricPresentation(
          title: "Purchase", value: "Available",
          help: "The provider reports that this account can purchase credits. This app does not make purchases."))
    }
  ...
  public static func creditsPresentation(
    ...
    if let credits {
      ...
      if let local = credits.approxLocalMessages {
        metrics.append(
          UsageMetricPresentation(
            title: "Local messages", value: "~\(local.lowerBound)–\(local.upperBound)",
            help: "Estimated local task messages the balance covers.", isSupportingDetail: true))
      }
      if let cloud = credits.approxCloudMessages {
        metrics.append(
          UsageMetricPresentation(
            title: "Cloud messages", value: "~\(cloud.lowerBound)–\(cloud.upperBound)",
            help: "Estimated cloud task messages the balance covers.", isSupportingDetail: true))
      }
    }
    if let resetCredits {
      metrics.append(
      ...
      if let expiry = resetCredits.expiries.map(\.expiresAt).min() {
        metrics.append(
          UsageMetricPresentation(
            title: "Next reset-credit expiry", value: expiry.formatted(date: .abbreviated, time: .shortened),
            help: "The earliest reported expiry among available reset credits. Expired credits cannot reset a limit."))
      }
      if resetCredits.immediatePurchaseEligible {
        metrics.append(
          UsageMetricPresentation(
            title: "Reset purchase", value: "Eligible",
            help: "The provider reports that an immediate reset can be purchased. This app does not make purchases."))
      }
      if let earned = resetCredits.totalEarned {
        metrics.append(
          UsageMetricPresentation(
            title: "Resets earned", value: "\(earned)", help: "Reset credits earned so far.", isSupportingDetail: true))
      }
    }
  ...
  static func chips(provider: ProviderID, snapshot: ProviderSnapshot?, hidePersonalInformation: Bool = false) -> [Chip]
  {
  ...
  public static func spendSummary(_ spend: SpendControl) -> String {
    var parts: [String] = []
    if spend.enabled == false { parts.append(spend.disabledReason.map { "Off (\(Format.humanize($0)))" } ?? "Off") }
    if spend.enabled == nil { parts.append("Enablement not reported") }
    let used = spend.creditAllowance?.used.map(CreditAllowance.formatted) ?? spend.used?.formatted
    let limit = spend.creditAllowance?.limit.map(CreditAllowance.formatted) ?? spend.limit?.formatted
    if used != nil || limit != nil { parts.append("\(used ?? "Unavailable") of \(limit ?? "unreported limit")") }
    if let percent = spend.percent { parts.append("\(Format.percent(percent)) used") }
  ...
  public static func creditsSummary(_ credits: CreditBalance) -> String {
    ...
    guard credits.balance != nil else { return "Balance not reported" }
