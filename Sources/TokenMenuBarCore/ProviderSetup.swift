...
public struct CredentialReadFailure: Error, Sendable, Equatable, CustomStringConvertible {
  ...
  public init(source: CredentialSource, error: any Error) {
    if let failure = error as? CredentialReadFailure {
      ...
      self.init(source: source, detail: credentialReadDetail(error))
    }
  }
...
private func credentialReadDetail(_ error: any Error) -> String {
  switch error {
  case CredentialStoreError.keychain(let status) where KeychainCredentialClient.deniedStatuses.contains(status):
    "Keychain access denied; allow it in Keychain Access or sign in again."
  case CredentialStoreError.keychain:
    "The Keychain item could not be read."
  case CredentialStoreError.malformed(let detail), CredentialStoreError.incomplete(let detail):
    detail
  case is CocoaError:
...
extension ProviderID {
  ...
  public var setup: ProviderSetupMetadata {
    ...
    case .claude:
      ...
      detail = "Run claude auth login and complete its sign-in. Token Menu Bar reads that local session."
    ...
    case .antigravity:
      sources = [
        CredentialSource(
          id: "antigravity.keychain", provider: self, title: "Antigravity Keychain",
          detail: "The Google sign-in that Antigravity and agy store in the gemini Keychain item.")
      ]
      title = "Sign in to Antigravity"
      detail =
        "Sign in to the Antigravity IDE or run agy once. Token Menu Bar reads the token they store in the Keychain."
      command = "agy"
    case .cursor:
      sources = [
        CredentialSource(
