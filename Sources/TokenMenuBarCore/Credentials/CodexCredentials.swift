...
public struct CodexAuth: Sendable, Equatable {
  ...
  public var supportsUsageAPI: Bool {
    document["tokens"]?["access_token"]?.stringValue != nil
  }
...
extension CodexAuthStore {
  public var localDataRoot: URL? { nil }
  ...
  public func save(_ auth: CodexAuth, replacing expected: CodexAuth) throws -> CredentialSaveResult<CodexAuth> {
    ...
    guard current?.auth == expected else {
      guard let current else { return .removed }
      return .changed(current.auth, source: current.source)
    }
    try save(auth)
    return .saved
  }
...
public struct FileCodexAuthStore: CodexAuthStore {
  ...
  public func save(_ auth: CodexAuth) throws {
    try writeCredentialData(JSONEncoder().encode(auth.document), to: url)
  }
}
...
public enum CodexHomeDiscovery {
  /// The configured home plus the directories the CLI is commonly pointed at, which a GUI launch cannot learn from
  /// `CODEX_HOME`. The CLI config names no alternate home, so nothing is read from config.toml here.
  public static func homes(configured: URL, home: URL, fileManager: FileManager = .default) -> [URL] {
    var seen: Set<String> = []
    return [configured, home.appendingPathComponent(".codex"), home.appendingPathComponent(".config/codex")]
      .filter { candidate in
        seen.insert(candidate.standardizedFileURL.path).inserted
          && (candidate == configured || fileManager.fileExists(atPath: candidate.path))
      }
  }
}

public enum CodexAuthStores {
  public static func store(codexHome: URL, keychain: KeychainCredentialClient) -> any CodexAuthStore {
    let file = FileCodexAuthStore(url: codexHome.appendingPathComponent("auth.json"))
    return switch CodexCredentialStorageReader.load(from: codexHome.appendingPathComponent("config.toml")) {
    case .file: file
    case .automatic, .keyring, .unknown:
      ChainedCodexAuthStore([KeychainCodexAuthStore(codexHome: codexHome, keychain: keychain), file])
    }
  }

  public static func discovering(
    configured: URL, home: URL, keychain: KeychainCredentialClient, fileManager: FileManager = .default
  ) -> any CodexAuthStore {
    let homes = CodexHomeDiscovery.homes(configured: configured, home: home, fileManager: fileManager)
    guard homes.count > 1 else { return store(codexHome: configured, keychain: keychain) }
    return DiscoveredCodexAuthStore(
      homes.map { CodexHomeAuthStore(home: $0, store: store(codexHome: $0, keychain: keychain)) })
  }
}

public struct CodexHomeAuthStore: Sendable {
  public let home: URL
  public let store: any CodexAuthStore

  public init(home: URL, store: any CodexAuthStore) {
    self.home = home
    self.store = store
  }
}

public struct CodexHomeAccount: Sendable, Equatable {
  public let home: URL
  public let auth: CodexAuth
  public let source: CredentialSource

  public init(home: URL, auth: CodexAuth, source: CredentialSource) {
    self.home = home
    self.auth = auth
    self.source = source
  }
  ...
  public var label: String {
    auth.email ?? (home.path as NSString).abbreviatingWithTildeInPath
  }
}

public final class DiscoveredCodexAuthStore: CodexAuthStore, @unchecked Sendable {
  public let stores: [CodexHomeAuthStore]
  private let lock = NSLock()
  private var selectedHome: URL?

  public var localDataRoot: URL? { lock.withLock { selectedHome } }

  public init(_ stores: [CodexHomeAuthStore]) {
    self.stores = stores
  }

  public var description: String { stores.map(\.store.description).joined(separator: ", ") }

  public var source: CredentialSource {
    CredentialSource(
      id: "codex.automatic", provider: .codex, title: "Codex credentials",
      detail: stores.map { ($0.home.path as NSString).abbreviatingWithTildeInPath }.joined(separator: ", "))
  }

  public func accounts() throws -> [CodexHomeAccount] {
    var accounts: [CodexHomeAccount] = []
    var firstError: CredentialReadFailure?
    for entry in stores {
      do {
        if let found = try entry.store.loadWithSource() {
          accounts.append(CodexHomeAccount(home: entry.home, auth: found.auth, source: found.source))
        }
      } catch {
        firstError = firstError ?? CredentialReadFailure(source: entry.store.source, error: error)
      }
    }
    if accounts.isEmpty, let firstError { throw firstError }
    return accounts
  }
  ...
  public func loadWithSource() throws -> (auth: CodexAuth, source: CredentialSource)? {
    try selected().map {
      ($0.account.auth, summarySource($0.account.source, count: $0.count, showing: $0.account.label))
    }
  }

  public func save(_ auth: CodexAuth) throws {
    let selectedHome = try selected()?.account.home
    let target = stores.first { $0.home == selectedHome } ?? stores.first
    guard let target else { return }
    try target.store.save(auth)
  }

  private func selected() throws -> (account: CodexHomeAccount, count: Int)? {
    let accounts = try accounts()
    let home = lock.withLock {
      if selectedHome == nil { selectedHome = accounts.first?.home }
      return selectedHome
    }
    return accounts.first { $0.home == home }.map { ($0, accounts.count) }
  }
}
