...
public actor UsageHistoryStore {
  ...
  public init(url: URL?, retentionDays: Int = defaultRetentionDays) throws {
    ...
    // SQLite ignores this once the file has pages, so it must precede the WAL switch to take effect on a new file.
  ...
  public func record(_ snapshot: ProviderSnapshot, now: Date) throws -> Int {
    if let fingerprint = snapshot.accountFingerprint {
      try activateAccount(snapshot.provider, fingerprint: fingerprint)
    }
  ...
  public func record(_ analytics: ProviderAnalytics) throws -> Int {
    ...
    try pruneIfNeeded(now: analytics.fetchedAt)
    if let fingerprint = analytics.accountFingerprint {
      try activateAccount(analytics.provider, fingerprint: fingerprint)
    }
    try database.withTransaction {
  ...
  public func exportCSV(to url: URL, hidePersonalInformation: Bool = false) async throws {
    try await withTaskCancellationHandler {
      try writeCSV(
        to: url,
        ...
        try database.forEachRow(
          ...
          try write(
            [
              "analytics", "", "", "", "", "", row.text(0), row.text(1), row.text(2),
              ProjectIdentity.exportLabel(
                row.text(3), metric: row.text(2), hidePersonalInformation: hidePersonalInformation),
  ...
  public func exportCSV(
    ...
    providers: [ProviderID]? = nil, includesEnd: Bool = true, hidePersonalInformation: Bool = false
  ) async throws {
    try await withTaskCancellationHandler {
      switch metric {
      ...
      case .analytics(let analyticsMetric):
        ...
        try exportAnalyticsCSV(
          ...
          before: exclusiveEnd, hidePersonalInformation: hidePersonalInformation)
      }
    } onCancel: {
      database.interrupt()
    }
  ...
  public func clear() throws -> Int {
    let removed = try database.withTransaction {
      ...
      let analytics = database.changes
      try database.execute("DELETE FROM archived_samples")
      let archivedSamples = database.changes
      try database.execute("DELETE FROM archived_analytics")
      let archivedAnalytics = database.changes
      try database.execute("DELETE FROM analytics_accounts")
      return samples + analytics + archivedSamples + archivedAnalytics
    }
  ...
  private func applyRetentionDays(_ days: Int, now: Date) throws -> HistoryPruneResult {
    ...
    let result = try database.withTransaction {
      ...
      let analytics = database.changes
      try database.execute("DELETE FROM archived_samples WHERE ts < ?", [.real(cutoff.timeIntervalSince1970)])
      let archivedSamples = database.changes
      try database.execute("DELETE FROM archived_analytics WHERE day < ?", [.text(analyticsCutoff)])
      let result = HistoryPruneResult(samples: samples + archivedSamples, analytics: analytics + database.changes)
      if result.total > 0 { try database.execute("PRAGMA incremental_vacuum(256)") }
      return result
    }
    retentionDays = clamped
  ...
  private static func migrate(_ database: SQLiteDatabase) throws {
    let version = try database.query("PRAGMA user_version") { $0.int(0) }.first!
    guard version <= schemaVersion else { throw SQLiteError.schema(version) }
    if version == 0 { try migrateToVersion1(database) }
    if version < 2 {
      try database.withTransaction {
        try database.execute(
          "CREATE TABLE archived_samples (fingerprint TEXT NOT NULL, ts REAL NOT NULL, key TEXT NOT NULL, label TEXT NOT NULL, used REAL NOT NULL, resets_at REAL, PRIMARY KEY(fingerprint, key, ts))"
        )
        try database.execute(
          "CREATE TABLE archived_analytics (fingerprint TEXT NOT NULL, provider TEXT NOT NULL, day TEXT NOT NULL, metric TEXT NOT NULL, series TEXT NOT NULL, value REAL NOT NULL, PRIMARY KEY(fingerprint, provider, day, metric, series))"
        )
        try database.execute("CREATE INDEX archived_samples_ts ON archived_samples(ts)")
        try database.execute("CREATE INDEX archived_analytics_day ON archived_analytics(day)")
        try database.execute("PRAGMA user_version = 2")
      }
    }
  }

  public func activateAccount(_ provider: ProviderID, fingerprint: String) throws {
    let stored = try database.query(
      "SELECT fingerprint FROM analytics_accounts WHERE provider = ?", [.text(provider.rawValue)]
    ) { $0.text(0) }.first
    guard stored != fingerprint else { return }
    try database.withTransaction {
      if let stored {
        let keyPattern = "\(provider.rawValue):*"
        try database.execute(
          "INSERT OR REPLACE INTO archived_samples SELECT ?, ts, key, label, used, resets_at FROM samples WHERE key GLOB ?",
          [.text(stored), .text(keyPattern)])
        try database.execute(
          "INSERT OR REPLACE INTO archived_analytics SELECT ?, provider, day, metric, series, value FROM analytics WHERE provider = ?",
          [.text(stored), .text(provider.rawValue)])
        try database.execute("DELETE FROM samples WHERE key GLOB ?", [.text(keyPattern)])
        try database.execute("DELETE FROM analytics WHERE provider = ?", [.text(provider.rawValue)])
        try database.execute(
          "INSERT OR REPLACE INTO samples SELECT ts, key, label, used, resets_at FROM archived_samples WHERE fingerprint = ? AND key GLOB ?",
          [.text(fingerprint), .text(keyPattern)])
        try database.execute(
          "INSERT OR REPLACE INTO analytics SELECT provider, day, metric, series, value FROM archived_analytics WHERE fingerprint = ? AND provider = ?",
          [.text(fingerprint), .text(provider.rawValue)])
        try database.execute(
          "DELETE FROM archived_samples WHERE fingerprint = ? AND key GLOB ?", [.text(fingerprint), .text(keyPattern)])
        try database.execute(
          "DELETE FROM archived_analytics WHERE fingerprint = ? AND provider = ?",
          [.text(fingerprint), .text(provider.rawValue)])
      }
      try database.execute(
        "INSERT OR REPLACE INTO analytics_accounts VALUES (?, ?)", [.text(provider.rawValue), .text(fingerprint)])
    }
    lastRecorded = lastRecorded.filter { $0.key.provider != provider }
  }

  private static func migrateToVersion1(_ database: SQLiteDatabase) throws {
    try database.withTransaction {
      try database.execute(
        """
      ...
      try database.execute("CREATE INDEX IF NOT EXISTS analytics_day ON analytics (day)")
