...
public enum PopoverGeometry {
  ...
  public static func maxSize(
    ...
    // An unattached status item must not force a small layout that is discarded when its real anchor arrives.
    let fallbackHeight = max(visibleFrame.height - margin - chromeHeight, 0)
    return CGSize(
