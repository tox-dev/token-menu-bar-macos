...
public enum UsageValueAppearance: Sendable {
  case current
  case stale

  public init(availability: QuotaAvailability, hasSnapshot: Bool) {
    self = availability != .current && hasSnapshot ? .stale : .current
  }

  public var foreground: SemanticColorRole { self == .stale ? .tertiary : .primary }

  public func accessibilityValue(_ value: String) -> String {
    self == .stale ? "Stale, last known: \(value)" : value
  }
}
