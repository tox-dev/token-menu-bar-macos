...
public actor GeminiProvider: UsageProvider {
  ...
  private var refreshRejection = RefreshRejection<GeminiAuth>()
  ...
  public func fetch(now: Date, options: FetchOptions) async -> ProviderFetchResult {
    ...
    do {
      ...
      return ProviderFetchResult(outcome: .notAuthenticated("Cannot read Gemini credentials: \(failure.detail)"))
    ...
    do {
      ...
      if case .http(let status, let body, _) = error, status == 403, GeminiAPI.isSubscriptionRequired(body) {
        return ProviderFetchResult(
    ...
    let snapshot = ProviderSnapshot(
      ...
      fetchedAt: now,
      details: GeminiMapper.details(quota, assist: assist)
    )
  ...
  {
    if let cachedAssist, cachedAssist.fingerprint == fingerprint {
      let hasBalances =
        !(cachedAssist.assist.paidTier?.availableCredits ?? []).isEmpty
        || !(cachedAssist.assist.currentTier?.availableCredits ?? []).isEmpty
      if now.timeIntervalSince(cachedAssist.at) < (hasBalances ? 60 : 3600) { return cachedAssist.assist }
    }
    let data = try await client.post(
