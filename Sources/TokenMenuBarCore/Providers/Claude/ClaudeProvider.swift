...
public actor ClaudeProvider: UsageProvider {
  ...
  private var refreshRejection = RefreshRejection<ClaudeOAuthCredentials>()
  ...
  public nonisolated func credentialState(now: Date) -> CredentialState {
    do {
      ...
      return .missing(CredentialReadFailure(source: credentials.source, error: error).detail)
    }
  }
  ...
  public func fetch(now: Date, options: FetchOptions) async -> ProviderFetchResult {
    ...
    do {
      ...
      let failure = CredentialReadFailure(source: credentials.source, error: error)
      return ProviderFetchResult(outcome: .notAuthenticated("Cannot read Claude credentials: \(failure.detail)"))
    ...
    if !matchesLocalScope {
      warnings.append(
        "Local transcript totals are unavailable for the selected Claude configuration. Its Keychain entry does not identify the transcript directory."
      )
    }
    warnings += transcript?.pricingWarnings ?? []
    switch usageResult {
    case .success(let response):
      if !response.malformedFields.isEmpty {
        warnings.append(
          "Claude returned unreadable fields: \(response.malformedFields.joined(separator: ", ")). Other reported values are shown."
        )
      }
      if response.droppedLimitCount > 0 {
        let noun = response.droppedLimitCount == 1 ? "limit" : "limits"
        warnings.append("Skipped \(response.droppedLimitCount) malformed Claude usage \(noun).")
      }
      ...
      let snapshot = ProviderSnapshot(
        ...
        identity: identity,
        windows: windows,
        spend: ClaudeMapper.spend(response),
        notices: ClaudeMapper.notices(response, now: now),
  ...
  private func refresh(
    ...
    let token: ClaudeAPI.TokenResponse
    do {
      ...
      token = try client.decode(ClaudeAPI.TokenResponse.self, data, operation: "claude.refresh")
    } catch {
