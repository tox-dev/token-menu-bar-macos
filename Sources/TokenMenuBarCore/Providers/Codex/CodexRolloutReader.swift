...
private struct RolloutCache {
  ...
  enum State {
    case candidates
    case searched(CodexRolloutReader.Reading?)
  }

  var hasSearchResult: Bool {
    if case .searched = state { return true }
    return false
  }

  var reading: CodexRolloutReader.Reading? {
    guard case .searched(let reading) = state else { return nil }
    return reading
  }
}
...
public actor CodexRolloutReader {
  ...
  public static let defaultCacheInterval: TimeInterval = 15 * 60
  ...
  private var cache: RolloutCache?
  ...
  func latest(now: Date = Date()) -> Reading? {
    // On battery saver or under thermal pressure an expired cache keeps serving; only the known files are re-checked.
    let throttled = powerState().throttlesScanning
    if let cache, cache.expiresAt > now || throttled, cache.hasSearchResult {
      if let refreshed = refreshedFiles(cache.files) {
        if refreshed == cache.files { return cache.reading }
        self.cache = RolloutCache(files: refreshed, expiresAt: cache.expiresAt, state: .candidates)
        search = RolloutSearch(files: refreshed, referenceNow: now, expiresAt: cache.expiresAt)
      } else {
        self.cache = nil
        treeScan = nil
        search = nil
    ...
    if treeScan == nil, search == nil {
      if let cache, cache.expiresAt > now || throttled {
        search = RolloutSearch(files: cache.files, referenceNow: now, expiresAt: cache.expiresAt)
      } else {
  ...
  private func advanceTreeScan(deadline: ContinuousClock.Instant) {
    ...
    while entriesRemaining > 0, clock.now < deadline {
      guard let url = active.enumerator.nextObject() as? URL else {
        ...
        let expiresAt = active.referenceNow.addingTimeInterval(cacheInterval)
        cache = RolloutCache(files: files, expiresAt: expiresAt, state: .candidates)
        treeScan = nil
        search = RolloutSearch(files: files, referenceNow: active.referenceNow, expiresAt: expiresAt)
        return
      }
  ...
  private func runSearchSlice(deadline: ContinuousClock.Instant) {
    ...
    while active.fileIndex < active.files.count, bytesRemaining > 0, clock.now < deadline {
      ...
      if let found {
        cache = RolloutCache(files: active.files, expiresAt: active.expiresAt, state: .searched(found))
      ...
      if reachedStaleEOF {
        ...
        if !current.oversized, let reading = Self.parse(reversedLine: current.reversedLine) {
          cache = RolloutCache(files: active.files, expiresAt: active.expiresAt, state: .searched(reading))
          workload.searchesCompleted += 1
          search = nil
          return
        }
    ...
    if active.fileIndex >= active.files.count {
      cache = RolloutCache(files: active.files, expiresAt: active.expiresAt, state: .searched(nil))
      workload.searchesCompleted += 1
      search = nil
    } else {
      search = active
