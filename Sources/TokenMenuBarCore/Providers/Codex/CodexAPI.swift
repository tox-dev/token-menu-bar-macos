...
enum CodexMapper {
  ...
  static func analytics(_ endpoint: CodexAPI.Analytics, rows: [JSONValue]) -> [AnalyticsPoint] {
    rows.flatMap { row -> [AnalyticsPoint] in
      ...
      case .plugins:
        return (row["plugin_usage_overviews"]?.arrayValue ?? []).compactMap { plugin in
          let name = plugin["plugin_name"]?.stringValue ?? plugin["display_name"]?.stringValue
          return name.flatMap { series in
            plugin["invocation_counts"]?.doubleValue.map {
              AnalyticsPoint(day: day, metric: .pluginInvocations, series: series, value: $0)
            }
          }
        }
      case .codeReview:
