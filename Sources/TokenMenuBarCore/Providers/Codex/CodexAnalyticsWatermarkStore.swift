...
public struct CodexAnalyticsWatermarkPersistence: @unchecked Sendable {
  public static let storageKey = "codexAnalyticsWatermarks"
...
struct CodexAnalyticsWatermarkStore {
  ...
  func clear() {
    persistence.defaults.removeObject(forKey: CodexAnalyticsWatermarkPersistence.storageKey)
  }
  ...
  private func decoded(now: Date) -> (StoredState, Bool) {
    guard let data = persistence.defaults.data(forKey: CodexAnalyticsWatermarkPersistence.storageKey) else {
  ...
  private func write(_ state: StoredState) {
    persistence.defaults.set(try! JSONEncoder().encode(state), forKey: CodexAnalyticsWatermarkPersistence.storageKey)
  }
}
