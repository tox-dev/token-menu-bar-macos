...
public struct VerificationProfile: Equatable, Sendable {
  ...
  public static let appearanceEnvironmentKey = "TOKEN_MENU_BAR_VERIFY_APPEARANCE"
  ...
  public init(
    fixture: Fixture = .standard, visibleFrameWidth: Double? = nil, nativePanels: Bool = false,
  ...
  public func clock(using system: Clock = .system) -> Clock {
    guard fixture == .longText else { return system }
    let now = system.now()
    return Clock(now: { now }, sleep: system.sleep)
  }
  ...
  init(environment: [String: String]) {
    ...
    appearance = environment[Self.appearanceEnvironmentKey].flatMap(Appearance.init(rawValue:))
  }
}
...
public struct LaunchPolicy: Equatable, Sendable {
  ...
  public init(
    ...
    guard
      verificationOnly || arguments.contains(Self.verificationArgument)
        || environment[Self.verificationEnvironmentKey] != nil
    else {
      mode = .standard
  ...
  public func relaunchArguments(detailedLogging: Bool) -> [String] {
    relaunchArguments + (detailedLogging ? ["-detailedLogging", "YES"] : [])
  }
