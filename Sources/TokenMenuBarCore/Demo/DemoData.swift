...
public enum DemoData {
  ...
  public static func analytics(_ provider: ProviderID, now: Date, days: Int) -> ProviderAnalytics? {
  ...
  public static func seed(_ history: UsageHistoryStore, providers: [ProviderID], now: Date) async throws {
...
public struct DemoProvider: UsageProvider {
  ...
  public func fetch(now: Date, options: FetchOptions) async -> ProviderFetchResult {
    ProviderFetchResult(
      outcome: .success(DemoData.snapshot(id, now: now)),
      analytics: options.includeAnalytics ? DemoData.analytics(id, now: now, days: options.analyticsDays) : nil
    )
  }
}
