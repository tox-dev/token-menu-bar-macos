...
public enum DemoData {
  ...
  public static func snapshot(
    ...
    let snapshot =
      ...
      case .antigravity: antigravity(now: now)
  ...
  public static func analytics(
    ...
    for (index, day) in stamps.enumerated() {
      ...
      case .gemini, .antigravity, .cursor, .copilot: break
      }
    }
    if fixture == .longText {
      points = points.map {
  ...
  static func antigravity(now: Date) -> ProviderSnapshot {
    let geminiSession = window(
      id: "gemini:session", label: "Gemini 5-hour", group: .session, duration: 5 * 3600, offset: 0, pace: 0.75, now: now
    )
    let geminiWeekly = window(
      id: "gemini:weekly", label: "Gemini weekly", group: .weekly, duration: 7 * 86400, offset: 4 * 86400 + 12 * 3600,
      pace: 0.6, now: now)
    let thirdPartySession = window(
      id: "3p:session", label: "Claude and GPT 5-hour", group: .session, duration: 5 * 3600, offset: 0, pace: 1.05,
      now: now)
    let thirdPartyWeekly = window(
      id: "3p:weekly", label: "Claude and GPT weekly", group: .weekly, duration: 7 * 86400,
      offset: 4 * 86400 + 12 * 3600, pace: 0.85, now: now)
    return ProviderSnapshot(
      provider: .antigravity,
      identity: ProviderIdentity(planName: "Google AI Ultra", tier: "google_ai_ultra", email: email),
      windows: [geminiSession, geminiWeekly, thirdPartySession, thirdPartyWeekly], fetchedAt: now)
  }
...
public struct DemoProvider: UsageProvider {
  ...
  public func credentialHealth(now: Date) async -> ProviderCredentialHealth {
    .valid(source: credentialSource, expiresAt: nil)
  }

  public var credentialSource: CredentialSource {
    CredentialSource(
      id: "demo-\(id.rawValue)", provider: id,
      title: fixture == .longText ? "Deterministic verification credential source for \(id.displayName)" : "Demo data",
      detail: fixture == .longText ? credentialDescription : "Generated locally for previewing the interface.")
  }
  ...
  private var controlAuditIssue: ProviderRecoveryIssue {
    ...
    case .antigravity:
      ProviderRecoveryIssue(
        kind: .credentialMissing, title: "Antigravity sign-in required",
        detail: "Run the deterministic verification command.", action: .copyCommand("agy"))
    case .cursor:
      ProviderRecoveryIssue(
