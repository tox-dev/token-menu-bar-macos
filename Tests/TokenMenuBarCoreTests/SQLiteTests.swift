...
@Test func sqliteWaitsForTransientWriterContention() throws {
  let database = try SQLiteDatabase(path: ":memory:")

  #expect(try database.query("PRAGMA busy_timeout") { $0.int(0) } == [5000])
}
...
@Test func sqliteReportsWhyADatabaseCannotOpen() {
  let missingDirectory = temporaryDirectory().appendingPathComponent("missing/usage.sqlite")

  #expect(throws: SQLiteError.self) { try SQLiteDatabase(path: missingDirectory.path) }
}
