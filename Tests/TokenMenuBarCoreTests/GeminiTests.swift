...
@Test func geminiKeepsDistinctCreditsAndUnspecifiedQuotaUnits() async throws {
  let snapshot = try #require(
    await geminiSnapshot(
      assist: .text(
        #"{"currentTier":{"id":"standard-tier","availableCredits":[{"creditType":"B","creditAmount":"2"},{"creditType":"A","creditAmount":"1"}]},"cloudaicompanionProject":"fixture"}"#
      ),
...
@Test func geminiIgnoresUnusableCreditEntriesWithoutLosingTheValidBalance() async throws {
  let snapshot = try #require(
    await geminiSnapshot(
      assist: .text(
        #"{"currentTier":{"id":"standard-tier","availableCredits":[{"creditAmount":"123"},{"creditType":"BAD","creditAmount":"not a number"},{"creditType":"GOOGLE_ONE_AI","creditAmount":"0"}]},"cloudaicompanionProject":"fixture"}"#
      ),
      quota: .json("gemini_quota")))
  #expect(snapshot.credits?.balance == 0)
  #expect(snapshot.details?.filter { $0.id.hasPrefix("credit:") }.count == 1)
}
...
@Test func geminiReportsOneWindowPerModelQuota() async throws {
  ...
  #expect(snapshot.windows.allSatisfy { $0.resetsAt == ISODate.parse("2026-08-30T07:00:00Z") && $0.duration == nil })
}
...
@Test func geminiReportsTheTierAndItsCredits() async throws {
  ...
  #expect(snapshot.credits?.balance == 300)
}
...
@Test func geminiProviderFetchesQuota() async throws {
  ...
  #if arch(arm64)
    #expect(quotaRequest.value(forHTTPHeaderField: "User-Agent")?.contains("darwin; arm64; cli") == true)
  #elseif arch(x86_64)
    #expect(quotaRequest.value(forHTTPHeaderField: "User-Agent")?.contains("darwin; x64; cli") == true)
  #endif
...
@Test func geminiFindsSubscriptionRequiredInErrorDetails() {
  let message = String(repeating: "account does not support this operation ", count: 8)
  let body =
    #"{"error":{"message":"\#(message)","status":"PERMISSION_DENIED","details":[{"reason":"SUBSCRIPTION_REQUIRED"}]}}"#
  #expect(GeminiAPI.isSubscriptionRequired(body))
}

@Test(
  arguments: [
    (#"{"error":{"status":"SUBSCRIPTION_REQUIRED"}}"#, true),
    ("prefix SUBSCRIPTION_REQUIRED suffix", true),
    (#"{"error":{"status":"PERMISSION_DENIED"}}"#, false),
    (#"{"status":"SUBSCRIPTION_REQUIRED"}"#, true),
  ])
func geminiClassifiesSubscriptionErrors(body: String, expected: Bool) {
  #expect(GeminiAPI.isSubscriptionRequired(body) == expected)
}
...
@Test func geminiProviderReportsRefreshFailures() async {
  ...
  transport.on(
    path: "/token",
    .text(#"{"error":"invalid_grant","error_description":"Token has been revoked."}"#, status: 401))
  ...
  #expect(
    revoked
      == "Gemini token refresh failed: the provider rejected the stored sign-in (invalid_grant). "
      + "Sign in with the CLI again.")
