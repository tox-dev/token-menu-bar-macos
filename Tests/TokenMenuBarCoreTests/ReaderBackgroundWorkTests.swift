...
@Test func replacingClaudeProviderReleasesItsThrottledReader() async throws {
  ...
  #expect(retainedReader.value == nil)
  _ = provider
}
...
@Test func replacingCodexProviderReleasesItsThrottledReader() async throws {
  ...
  var provider: CodexProvider? = CodexProvider(
    ...
    client: APIClient(transport: StubTransport(), log: makeLog(), clock: testClock), log: makeLog(),
    allowRefresh: { false }, analyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence(defaults: testDefaults())
  )
  _ = await reader?.latest(now: fixedNow)
  #expect(provider != nil)
  reader = nil
...
private func waitForRelease<Value: AnyObject>(_ reference: WeakReference<Value>) async throws {
  // Actor jobs may still be finishing; release must precede the 60-second throttle.
