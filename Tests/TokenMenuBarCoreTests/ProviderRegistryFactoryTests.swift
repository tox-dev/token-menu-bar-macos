...
@Test @MainActor func registryRetryInvalidatesOnlyProviderCredentialServices() async {
  let services = [
    ClaudeOAuthCredentials.keychainService, KeychainCodexAuthStore.service, KeychainGeminiAuthStore.service,
    KeychainAntigravityAuthStore.service, KeychainCopilotAuthStore.service,
  ]
  await confirmation("Credential service invalidated", expectedCount: services.count) { invalidated in
    let keychain = KeychainCredentialClient(
      load: { _, _ in nil }, save: { _, _, _ in },
      invalidate: { service, variants in
        #expect(services.contains(service))
        #expect(variants == (service == ClaudeOAuthCredentials.keychainService))
        invalidated()
      })
    let root = temporaryDirectory()
    let registry = ProviderRegistryFactory.make(
      configuration: ProviderRegistryFactory.Configuration(
        home: root, supportDirectory: root.appendingPathComponent("support"), environment: [:], userName: "fixture",
        resourceURLs: [:], enabledProviders: [], keychain: keychain, readOAuthSource: { _ in nil },
        allowTokenRefresh: { false },
        analyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence(defaults: testDefaults())),
      client: APIClient(transport: StubTransport(), log: makeLog()), log: makeLog())
    registry.retryCredentialReads(Set(ProviderID.allCases))
  }
}
...
@Test @MainActor func providerRegistryFactoryBuildsExactCredentialChainsAndSetup() async throws {
  ...
  let registry = ProviderRegistryFactory.make(
    configuration: ProviderRegistryFactory.Configuration(
      ...
      keychain: MemoryKeychain().client, readOAuthSource: { _ in nil },
      allowTokenRefresh: { false },
      analyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence(defaults: testDefaults())),
...
func providerRegistryFactoryResolvesGeminiOAuthWhenItsProviderRefreshes(environmentClient: Bool) async throws {
  let root = temporaryDirectory()
  let home = root.appendingPathComponent("home")
  let support = root.appendingPathComponent("support")
  let environment = [
  ...
  let registry = ProviderRegistryFactory.make(
    configuration: ProviderRegistryFactory.Configuration(
      ...
      readOAuthSource: { _ in Data("OAUTH_CLIENT_ID = 'factory-client'; OAUTH_CLIENT_SECRET = 'factory-secret';".utf8)
      },
      allowTokenRefresh: { true },
      analyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence(defaults: testDefaults())),
    client: APIClient(transport: transport, log: makeLog()),
...
@Test @MainActor func providerRegistryFactoryWiresAntigravityToTheKeychainAndProcessTable() async throws {
  let root = temporaryDirectory()
  ...
  let transport = StubTransport()
  transport.on(path: "LanguageServerService/RetrieveUserQuotaSummary", .json("antigravity_local_quota"))
  transport.on(path: "LanguageServerService/GetUserStatus", .json("antigravity_user_status"))
  let registry = ProviderRegistryFactory.make(
    configuration: ProviderRegistryFactory.Configuration(
      home: root,
      supportDirectory: root.appendingPathComponent("support"),
      environment: [:],
      userName: "factory-antigravity",
      resourceURLs: [:],
      enabledProviders: [.antigravity],
      keychain: keychain.client, readOAuthSource: { _ in nil },
      allowTokenRefresh: { false },
      analyticsWatermarkPersistence: CodexAnalyticsWatermarkPersistence(defaults: testDefaults()),
      processScanner: StubProcessScanner([antigravityServer], ports: [4242: [4321]])),
    client: APIClient(transport: transport, log: makeLog()),
    log: makeLog())
  let provider = try #require(registry[.antigravity])

  #expect(provider.credentialDescription == "Keychain item gemini (antigravity)")
  #expect(await provider.credentialHealth(now: fixedNow).source?.id == "antigravity.local")
  let result = await provider.fetch(now: fixedNow, options: FetchOptions())
  #expect(result.outcome.snapshot?.identity?.email == "you@example.com")
  #expect(transport.requests.first?.url?.port == 4321)
}

@Test @MainActor func providerRegistryFactoryResolvesAntigravityOAuthWhenItsProviderRefreshes() async throws {
  let root = temporaryDirectory()
  let keychain = MemoryKeychain()
  let expired = AntigravityAuth(
    accessToken: "expired", refreshToken: "refresh", expiresAt: fixedNow.addingTimeInterval(-60))
  try keychain.client.save(
    antigravityKeychainValue(
      .object([
        "access_token": .string(expired.accessToken), "refresh_token": .string(expired.refreshToken!),
        "expiry": .number(expired.expiresAt!.timeIntervalSince1970),
  ...
  let registry = ProviderRegistryFactory.make(
    configuration: ProviderRegistryFactory.Configuration(
      ...
      supportDirectory: root.appendingPathComponent("support"),
      environment: [
        "ANTIGRAVITY_OAUTH_CLIENT_ID": "factory-client", "ANTIGRAVITY_OAUTH_CLIENT_SECRET": "factory-secret",
      ],
      userName: "factory-antigravity",
      resourceURLs: [:],
      enabledProviders: [.antigravity],
  ...
  #expect(String(decoding: tokenRequest.httpBody!, as: UTF8.self).contains("client_id=factory-client"))
}
