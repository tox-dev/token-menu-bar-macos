...
@Test @MainActor func coverageClosureCoordinatorPreservesRateLimitStrikesAcrossRegistryReplacement() async throws {
  ...
  #expect(coordinator.nextAttempt(for: .codex) == fixedNow.addingTimeInterval(2 * RefreshCoordinator.rateLimitBackoff))
}
...
@Test @MainActor func coverageClosureCoordinatorCanSkipWidgetAndAdaptiveWidthPublication() throws {
  ...
  coordinator.rebuildStatus(now: fixedNow, publishWidget: true)

  #expect(publications == initialPublications + 1)
}
...
@Test @MainActor func coverageClosureCoordinatorLoopDoesNotRetainItsOwner() async throws {
  ...
  let wasReleased = { [weak coordinator] in coordinator == nil }
  coordinator?.start()
  coordinator = nil
