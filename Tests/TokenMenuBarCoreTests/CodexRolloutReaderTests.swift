...
func rolloutReaderServesTheExpiredCacheWhilePowerIsConstrained(throttled: PowerState) async throws {
  let root = temporaryDirectory()
  let file = root.appendingPathComponent("rollout-current.jsonl")
  try (rolloutLine(primary: 7) + "\n").write(to: file, atomically: true, encoding: .utf8)
  let power = PowerSwitch()
  let reader = CodexRolloutReader(sessionsRoot: root, cacheInterval: 300, powerState: power.read)
  #expect(await reader.latest(now: fixedNow)?.rateLimit.primaryWindow?.usedPercent == 7)

  power.current = throttled
  try (rolloutLine(primary: 42) + "\n").write(
    to: root.appendingPathComponent("rollout-newer.jsonl"), atomically: true, encoding: .utf8)
  #expect(await reader.latest(now: fixedNow.addingTimeInterval(301))?.rateLimit.primaryWindow?.usedPercent == 7)
  #expect((await reader.workload).treesScanned == 1)
  try (rolloutLine(primary: 55) + "\n").write(to: file, atomically: true, encoding: .utf8)
  try FileManager.default.setAttributes([.modificationDate: Date(timeIntervalSince1970: 1000)], ofItemAtPath: file.path)
  #expect(await reader.latest(now: fixedNow.addingTimeInterval(302))?.rateLimit.primaryWindow?.usedPercent == 55)
  #expect((await reader.workload).treesScanned == 1)

  power.current = PowerState(lowPowerMode: false, thermalPressure: false)
  #expect(await reader.latest(now: fixedNow.addingTimeInterval(303))?.rateLimit.primaryWindow?.usedPercent == 42)
  #expect((await reader.workload).treesScanned == 2)
}

@Test func rolloutReaderStillScansAnEmptyCacheOnLowPower() async throws {
  let root = temporaryDirectory()
  try (rolloutLine(primary: 7) + "\n").write(
    to: root.appendingPathComponent("rollout-current.jsonl"), atomically: true, encoding: .utf8)
  let reader = CodexRolloutReader(
    sessionsRoot: root, powerState: { PowerState(lowPowerMode: true, thermalPressure: true) })
  #expect(await reader.latest(now: fixedNow)?.rateLimit.primaryWindow?.usedPercent == 7)
  #expect(CodexRolloutReader.defaultCacheInterval == 15 * 60)
}
