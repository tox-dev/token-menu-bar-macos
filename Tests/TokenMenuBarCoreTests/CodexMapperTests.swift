...
@Test func codexKeepsQuotasWhenAdditionalModelsAreMalformed() async {
  let (result, _) = await codexFetch(
    usage: .text(#"{"rate_limit":{"primary_window":{"used_percent":8}},"additional_rate_limits":[false]}"#))
  #expect(result.outcome.snapshot?.windows.first?.usedPercent == 8)
  #expect(result.warnings.contains { $0.contains("1 unreadable Codex model limits") })
}

@Test(arguments: ["text", "title"])
func codexPreservesProviderUpsellText(field: String) async throws {
  let snapshot = try #require(
    await codexSnapshot(usage: .text(#"{"rate_limit_upsell":{"\#(field)":"More capacity is available"}}"#)))
  #expect(snapshot.notices.map(\.text) == ["More capacity is available"])
}

@Test(arguments: ["false", "[]"])
func codexAcceptsMissingReadableAdditionalLimits(value: String) async throws {
  let snapshot = try #require(await codexSnapshot(usage: .text(#"{"additional_rate_limits":\#(value)}"#)))
  #expect(snapshot.windows.isEmpty)
}

@Test func codexPreservesRelativeCreditAllowanceResets() async throws {
  let snapshot = try #require(
    await codexSnapshot(
      usage: .text(
        #"{"spend_control":{"individual_limit":{"remaining_percent":75,"reset_after_seconds":60}}}"#)))
  #expect(snapshot.spend?.percent == 25)
  #expect(snapshot.spend?.resetsAt == fixedNow.addingTimeInterval(60))
}

@Test func codexPreservesResetCreditDetailsWithoutAnIdentifier() async throws {
  let transport = StubTransport()
  transport.on(path: "/wham/usage", .text("{}"))
  transport.on(
    path: "rate-limit-reset-credits",
    .text(
      #"{"credits":[{"reset_type":"promo","status":"used","granted_at":"2026-08-01T00:00:00Z","expires_at":"2026-09-01T00:00:00Z"}]}"#
    ))
  let result = await codexProvider(MemoryCodexStore(validCodex), transport: transport)
    .fetch(now: fixedNow, options: FetchOptions())
  let detail = try #require(result.outcome.snapshot?.resetCredits?.details?.first)
  #expect(detail.id == "credit-0")
  #expect(detail.title == "Reset credit")
  #expect(detail.value.contains("Used · Promo") && detail.value.contains("Granted") && detail.value.contains("Expires"))
  #expect(result.outcome.snapshot?.resetCredits?.expiries.isEmpty == true)
}
...
@Test func codexReportsTheIndividualSpendLimit() async throws {
  ...
  #expect(
    spend.creditAllowance
      == CreditAllowance(used: Decimal(string: "42.5"), limit: 100, remaining: Decimal(string: "57.5")))
...
@Test func codexWarnsWhenACreditEventHasNoRecognizedAmount() async {
  let (result, _) = await codexFetch(usage: .json("codex_usage"), analytics: true) { transport in
    transport.on(path: "credit-usage-events", .text(#"{"data":[{"date":"2026-08-29","cost":3}]}"#))
    stubCodexAnalytics(transport)
  }
