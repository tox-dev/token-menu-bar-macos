...
func settingsModelItemsKeepProviderOrderAndDistinctStableIdentities(selected: Bool) {
  let window = presentationWindow("session", percent: 30)
  let claude = SettingsModelRow(
    key: WindowKey(.claude, window), window: window, detail: "session", recency: "today", isSelected: selected,
    defaultLabel: "CL", label: "CL", isLabelOverridden: false)
  let codex = SettingsModelRow(
    key: WindowKey(.codex, window), window: window, detail: "session", recency: "today", isSelected: selected,
    defaultLabel: "CX", label: "CX", isLabelOverridden: false)
  let claudeGroup = SettingsProviderGroup(
    provider: .claude, rows: [claude], selectedCount: selected ? 1 : 0, totalCount: 1)
  let codexGroup = SettingsProviderGroup(
    provider: .codex, rows: [codex], selectedCount: selected ? 1 : 0, totalCount: 1)
  let items = SettingsModelPresentation.items(in: [codexGroup, claudeGroup])
  #expect(items == [.provider(codexGroup), .model(codex), .provider(claudeGroup), .model(claude)])
  #expect(items.map(\.id) == ["provider:codex", "model:codex:session", "provider:claude", "model:claude:session"])
}

@Test func settingsModelItemsKeepAnEmptyFilteredListEmpty() {
  #expect(SettingsModelPresentation.items(in: []).isEmpty)
}
...
@Test(arguments: [("  NEW  ", "NEW"), ("", nil), ("SPARKLES", "SPARKL")])
func shortLabelPolicySavesValidDraftsAndClearsEmptyOverrides(draft: String, expected: String?) {
  let window = presentationWindow("session", percent: 20)
  let key = WindowKey(.claude, window)
  let labels = ShortLabelPolicy.validOverrides(
    windows: [key: window], persisted: [key: "OLD"], drafts: [key: draft])
  #expect(labels[key] == expected)
}
