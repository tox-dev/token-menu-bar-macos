...
@Test func rolloutReaderResumesFromCachedCandidatesAfterCancellation() async throws {
  let root = temporaryDirectory()
  let file = root.appendingPathComponent("rollout-current.jsonl")
  try (coverageRolloutLine(primary: 7) + "\n").write(to: file, atomically: true, encoding: .utf8)
  let reader = CodexRolloutReader(
    sessionsRoot: root, cacheInterval: 300, workByteBudget: 1, workTimeBudget: 2, backgroundWorkDelay: 60)
  #expect(await reader.latest(now: fixedNow) == nil)
  await reader.cancelBackgroundWork()

  var reading: CodexRolloutReader.Reading?
  for _ in 0...coverageRolloutLine(primary: 7).utf8.count {
    reading = await reader.latest(now: fixedNow.addingTimeInterval(1))
