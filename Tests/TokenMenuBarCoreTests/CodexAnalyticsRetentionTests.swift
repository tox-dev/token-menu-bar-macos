...
@Test func clearingCodexHistoryResetsCoverageAcrossRelaunch() async {
  let defaults = watermarkDefaults()
  let first = codexProvider(
    MemoryCodexStore(validCodex), transport: analyticsTransport(), analyticsWatermarkPersistence: persistence(defaults))
  _ = await first.fetch(now: fixedNow, options: FetchOptions(includeAnalytics: true, analyticsDays: 30))
  await first.resetAnalyticsHistory()
  let transport = analyticsTransport()
  let next = codexProvider(
    MemoryCodexStore(validCodex), transport: transport, analyticsWatermarkPersistence: persistence(defaults))
  _ = await next.fetch(
    now: fixedNow.addingTimeInterval(86400), options: FetchOptions(includeAnalytics: true, analyticsDays: 30))
  #expect(startDates(transport) == ["2026-08-01"])
}
...
@Test func codexAnalyticsWatermarksRecoverFromCorruptStorage() async {
  let defaults = watermarkDefaults()
  defaults.set(Data("not json".utf8), forKey: CodexAnalyticsWatermarkPersistence.storageKey)
  let firstTransport = analyticsTransport()
  _ = await codexProvider(
    MemoryCodexStore(validCodex), transport: firstTransport, analyticsWatermarkPersistence: persistence(defaults)
  ).fetch(now: fixedNow, options: FetchOptions(includeAnalytics: true, analyticsDays: 30))
...
@Test func codexAnalyticsMigratesLegacyWatermarksWithoutPersistingIdentityPlaintext() async throws {
  ...
  defaults.set(try JSONEncoder().encode(legacy), forKey: CodexAnalyticsWatermarkPersistence.storageKey)
  let transport = analyticsTransport()
  _ = await codexProvider(
    MemoryCodexStore(auth), transport: transport, analyticsWatermarkPersistence: persistence(defaults)
  ).fetch(now: fixedNow, options: FetchOptions(includeAnalytics: true, analyticsDays: 30))
  ...
  let stored = String(
    data: defaults.data(forKey: CodexAnalyticsWatermarkPersistence.storageKey)!, encoding: .utf8)!
...
@Test func codexAnalyticsWatermarksDiscardInvalidAndExpiredCoverage() throws {
  ...
  defaults.set(
    try JSONSerialization.data(withJSONObject: document), forKey: CodexAnalyticsWatermarkPersistence.storageKey)
  ...
  defaults.set(
    try JSONSerialization.data(withJSONObject: unknownVersion), forKey: CodexAnalyticsWatermarkPersistence.storageKey)
