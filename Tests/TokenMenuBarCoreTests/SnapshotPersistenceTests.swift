...
@Test @MainActor func snapshotPersistenceWritesFreshnessWithoutReloadingUnchangedContent() async throws {
  ...
  await persistence.flush()
  #expect(reloads.count == 1)
  let heartbeat = WidgetSnapshot(
    rows: WidgetSnapshot.placeholder.rows, attention: WidgetSnapshot.placeholder.attention,
    updatedAt: stored.updatedAt.addingTimeInterval(WidgetSnapshot.refreshInterval))
  await persistence.submitWidget(heartbeat)
  await persistence.flush()
  #expect(store.read()?.updatedAt == heartbeat.updatedAt)
  #expect(reloads.count == 1)
  await persistence.submitWidget(
    WidgetSnapshot(rows: heartbeat.rows, attention: !heartbeat.attention, updatedAt: heartbeat.updatedAt))
  await persistence.flush()
  #expect(reloads.count == 2)
  let workload = await persistence.workload
  #expect(workload.widgetWrites == 3)
  #expect(workload.widgetReloads == 2)
