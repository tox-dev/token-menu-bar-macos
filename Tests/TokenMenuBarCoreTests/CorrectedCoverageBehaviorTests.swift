...
func correctedCoverageCursorTreatsMissingSpendLimitsAsNotReached(body: String) async throws {
  ...
  #expect(snapshot.spend?.enabled == nil)
...
func correctedCoverageGeminiRejectsRefreshResponsesWithoutAnAccessToken(body: String) async {
  ...
  let detail = body.contains("invalid_grant") ? "invalid_grant" : "refresh returned no access token"
  #expect(result.outcome == .notAuthenticated("Gemini token refresh failed: \(detail)"))
