...
func hiddenHistoryOnlyPreloadsWhenRequested(preload: Bool) async throws {
  let history = try UsageHistoryStore(url: nil)
  try await history.record(
    ProviderAnalytics(
      provider: .codex,
      points: [AnalyticsPoint(day: DayStamp.string(fixedNow), metric: .turns, series: "model", value: 5)],
      fetchedAt: fixedNow))
  let presenter = HistoryPresenter(
    history: history, settings: Settings(defaults: testDefaults()), clock: testClock,
    initialMetric: .analytics(.turns))
  ...
  presenter.ensureLoaded(whileInactive: preload)
  await presenter.waitForLoad()

  #expect(presenter.state.data?.series.flatMap(\.points).map(\.value) == (preload ? [5] : nil))
}

@Test @MainActor func preloadedHistoryDoesNotReloadOnActivationWithoutARevision() async throws {
  let history = try UsageHistoryStore(url: nil)
  let presenter = HistoryPresenter(
    history: history, settings: Settings(defaults: testDefaults()), clock: testClock,
    initialMetric: .analytics(.turns))
  presenter.setActive(false)
  presenter.reload()
  presenter.ensureLoaded(whileInactive: true)
  await presenter.waitForLoad()
  #expect(presenter.state.data?.isEmpty == true)
  try await history.record(
    ProviderAnalytics(
