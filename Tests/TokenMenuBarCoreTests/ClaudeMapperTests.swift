...
@Test func claudeSkipsOnlyTheMalformedLimit() async throws {
  let usage = #"""
    ...
    """#
  let transport = StubTransport()
  transport.on(path: "/api/oauth/usage", .text(usage))
  transport.on(path: "/api/oauth/profile", .json("claude_profile"))
  let result = await claudeProvider(MemoryClaudeStore(validClaude), transport: transport).fetch(
    now: fixedNow, options: FetchOptions())
  #expect(result.outcome.snapshot?.windows.map(\.id) == ["session", "weekly"])
  #expect(result.warnings.contains("Skipped 1 malformed Claude usage limit."))
}

@Test func claudePreservesFlatLimitsWhenStructuredLimitsAreMalformed() async throws {
  let usage = #"{"limits": [{"kind": "session", "percent": null}], "five_hour": {"utilization": 99}}"#
  let snapshot = try #require(await claudeSnapshot(usage: .text(usage)))
  #expect(snapshot.windows.map(\.usedPercent) == [99])
}
...
@Test func claudeFallsBackToTheFlatWindowKeys() async throws {
  ...
  #expect(windows.first { $0.id == "seven_day_mystery" }?.label == "Mystery")
...
@Test func claudeReportsTheSpendBlockWithoutInventingAReset() async throws {
  let snapshot = try #require(await claudeSnapshot(usage: .json("claude_usage")))
  ...
  #expect(snapshot.spend?.resetsAt == nil)
}

@Test func claudeDerivesSpendFromExtraUsageWhenTheBlockIsMissing() async throws {
  let usage = #"""
    {"extra_usage": {"is_enabled": true, "monthly_limit": 8000, "used_credits": 2445, "currency": "EUR",
...
@Test func claudeReportsNoSpendWithoutSpendData() async throws {
  let snapshot = try #require(await claudeSnapshot(usage: .text(#"{"limits":[]}"#)))
