...
@Test @MainActor func notifierRequestsProvisionalAuthorizationWithoutAPrompt() async {
  let center = FakeNotificationCenter()
  let notifier = Notifier(center: center, log: makeLog())
  await notifier.requestAuthorization(provisional: true)
  await notifier.requestAuthorization()
  #expect(center.requestedOptions == [[.alert, .sound, .provisional], [.alert, .sound]])
}

@Test(arguments: [true, false])
@MainActor func notifierAttachesTheDefaultSoundOnlyWhenAsked(playSound: Bool) async {
  let center = FakeNotificationCenter()
  let notifier = Notifier(center: center, log: makeLog())
  await notifier.requestAuthorization()
  notifier.playSound = playSound
  await notifier.deliver([event(.threshold, id: "a")])
  #expect(center.requests[0].content.sound == (playSound ? UNNotificationSound.default : nil))
}

@Test @MainActor func notifierDropsAGroupedBannerWhenOneOfItsWindowsResets() async {
  let center = FakeNotificationCenter()
  let notifier = Notifier(center: center, log: makeLog())
  await notifier.requestAuthorization()
  let grouped = NotificationEvent(id: "claude:thresholds", kind: .threshold, provider: .claude, title: "t", body: "b")
  let other = NotificationEvent(id: "codex:thresholds", kind: .threshold, provider: .codex, title: "t", body: "b")
  await notifier.deliver([grouped, other])
  await notifier.deliver([event(.reset, id: "r")])
  #expect(center.removed == ["claude:thresholds"])
  #expect(notifier.delivered.map(\.id) == ["codex:thresholds", "r"])
}
