...
func supportingSummaryChangesOnlyResizeCollapsedDisclosures(expanded: Bool) async {
  let state = DisclosureState()
  state.setExpanded(expanded, for: "details")
  func details(summary: String?) -> some View {
    SupportingDetails("Collection", id: "details", state: state, summary: summary) { Text("Analytics controls") }
  }
  let hosting = host(details(summary: nil), width: 400, height: 200)
  let before = hosting.fittingSize.height
  hosting.rootView = details(summary: "Analytics every 20 min")
  await mainActorTurn()
  hosting.layoutSubtreeIfNeeded()
  let after = hosting.fittingSize.height
  #expect(expanded ? abs(after - before) < 1 : after > before + 10)
}
