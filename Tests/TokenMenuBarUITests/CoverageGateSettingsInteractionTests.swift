...
func coverageGateSettingsRespondsToAChangedProviderFocusRequest(mountsIncrementally: Bool) async throws {
  let environment = try makeEnvironment()
  let hosting = host(
    SettingsTab(environment: environment, providerFocusRequest: nil, mountsIncrementally: mountsIncrementally),
  ...
  hosting.rootView = SettingsTab(
    environment: environment, providerFocusRequest: request, mountsIncrementally: mountsIncrementally)
