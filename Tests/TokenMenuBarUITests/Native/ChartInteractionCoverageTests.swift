...
@Test @MainActor func chartDistinguishesAResetFromAnOrdinarySample() throws {
  let environment = try makeEnvironment(populate: false)
  func render(reset: Bool) throws -> Data {
    let model = HistoryChartModel(
      series: [
        HistorySeries(
          key: WindowKey(provider: .claude, windowID: "session"), label: "Session",
          points: [
            SeriesPoint(date: fixedNow.addingTimeInterval(-60), value: 80),
            SeriesPoint(date: fixedNow, value: 5, isReset: reset),
            SeriesPoint(date: fixedNow.addingTimeInterval(60), value: 10),
          ])
      ], domain: fixedNow.addingTimeInterval(-60)...fixedNow.addingTimeInterval(60), yMax: 100)
    return try #require(
      renderedChart(
        UsageChart(data: model, presenter: environment.historyPresenter, stacked: false, timeZone: .current)))
  }

  #expect(try render(reset: true) != render(reset: false))
}

@Suite(.serialized)
struct ChartInteractionCoverageTests {
  ...
  @Test @MainActor func inspectorIsolatesTheFocusedSeriesFromAKeyboardEvent() async throws {
    ...
    let hostingFixture = NativeHosting(
      HistoryInspector(environment: environment), width: 320, height: 400, styleMask: [.titled])
    defer { hostingFixture.close() }
    hostingFixture.show()
    await mainActorTurn()
    let hosting = hostingFixture.view
  ...
  @Test @MainActor func historyTabDefaultExportRequestsTheSavePanel() async throws {
    let environment = try makeEnvironment(populate: false)
    let export = try #require(chartCoverageButtons(in: HistoryTab(environment: environment).body).first)
    try await interceptFilePanelPresentation {
      export.action()
      #expect(await waitUntil { ChartCoverageSavePanelObservation.wasPresented })
    }
  }

  @Test(arguments: [false, true]) @MainActor
  func filePanelUsesASheetOnlyWhenItHasAParent(attached: Bool) async throws {
    let fixture = NativeHosting(Text("Export fixture"), width: 400, height: 240, styleMask: .titled)
    defer { fixture.close() }
    let window = try #require(fixture.view.window)
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("tmb-panel-\(UUID().uuidString)")
    try await interceptFilePanelPresentation {
      let result = await LiveDependencies.chosen({
        #expect(withUnsafeCurrentTask { $0 == nil })
        return LiveDependencies.exportPanel(default: directory)
      }) { panel, completion in
        #expect(withUnsafeCurrentTask { $0 == nil })
        LiveDependencies.presentFilePanel(panel, parent: attached ? window : nil, completion: completion)
      }
      #expect(result == nil)
      #expect(ChartCoverageSavePanelObservation.wasPresented)
      #expect(ChartCoverageSavePanelObservation.parent === (attached ? window : nil))
    }
  }

  @Test(arguments: [false, true]) @MainActor
  func defaultVerificationChoosersRequestNativePanels(directory: Bool) async throws {
    let support = FileManager.default.temporaryDirectory.appendingPathComponent("tmb-chooser-\(UUID().uuidString)")
    let profile = VerificationProfile(nativePanels: true)
    try await interceptFilePanelPresentation {
      let result: URL?
      if directory {
        result = await LiveDependencies.directoryChooser(
          profile: profile, paths: LiveDependencies.Paths(home: support, supportDirectory: support),
          supportDirectory: support)(ProviderID.claude.sandboxResources[0])
      } else {
        result = await LiveDependencies.exportChooser(profile: profile, supportDirectory: support)()
      }
      #expect(result == nil)
      #expect(ChartCoverageSavePanelObservation.wasPresented)
    }
...
@MainActor
private func interceptFilePanelPresentation(_ action: @MainActor () async throws -> Void) async throws {
  let original = try #require(
    class_getInstanceMethod(NSSavePanel.self, #selector(NSSavePanel.begin(completionHandler:))))
  let replacement = try #require(
    class_getInstanceMethod(NSSavePanel.self, #selector(NSSavePanel.chartCoverageBegin(completionHandler:))))
  let sheetOriginal = try #require(
    class_getInstanceMethod(NSSavePanel.self, #selector(NSSavePanel.beginSheetModal(for:completionHandler:))))
  let sheetReplacement = try #require(
    class_getInstanceMethod(NSSavePanel.self, #selector(NSSavePanel.chartCoverageBeginSheet(for:completionHandler:))))
  method_exchangeImplementations(original, replacement)
  method_exchangeImplementations(sheetOriginal, sheetReplacement)
  defer { method_exchangeImplementations(replacement, original) }
  defer { method_exchangeImplementations(sheetReplacement, sheetOriginal) }
  ChartCoverageSavePanelObservation.wasPresented = false
  ChartCoverageSavePanelObservation.parent = nil
  try await action()
}
...
@MainActor private enum ChartCoverageSavePanelObservation {
  ...
  static weak var parent: NSWindow?
}

extension NSSavePanel {
  @objc fileprivate func chartCoverageBegin(completionHandler: @escaping (NSApplication.ModalResponse) -> Void) {
  ...
  @objc fileprivate func chartCoverageBeginSheet(
    for window: NSWindow, completionHandler: @escaping (NSApplication.ModalResponse) -> Void
  ) {
    ChartCoverageSavePanelObservation.parent = window
    DispatchQueue.main.async {
      completionHandler(.cancel)
      ChartCoverageSavePanelObservation.wasPresented = true
    }
