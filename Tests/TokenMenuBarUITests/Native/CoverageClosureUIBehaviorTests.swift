...
func tabContainerUsesTheProposedViewport(size: CGSize) throws {
  prepareTestApp()
  let controller = NSHostingController(
    rootView: PersistentTabContent(
      environment: try makeEnvironment(), selection: .usage, chooseHistoryExportURL: { nil }, onMeasure: { _ in },
      onPresent: { _ in }))
  #expect(controller.sizeThatFits(in: size) == size)
}

@Test @MainActor func tabContainerIdealSizeDoesNotDependOnItsCachedTabs() throws {
  prepareTestApp()
  let controller = NSHostingController(
    rootView: PersistentTabContent(
      environment: try makeEnvironment(), selection: .usage, chooseHistoryExportURL: { nil }, onMeasure: { _ in },
...
@Test @MainActor func rootTabClickPresentsItsContentBeforeReturning() throws {
  ...
  let usage = try #require(container.accessibilityChildren()?.first as? NSView)

  control.selectedSegment = 2
  NSApp.sendAction(try #require(control.action), to: control.target, from: control)

  let settings = try #require(container.accessibilityChildren()?.first as? NSView)
...
@Test @MainActor func rootTabResizeKeepsTheOutgoingHostsCachedFrame() throws {
  ...
  defer { hostingFixture.close() }
  let hosting = hostingFixture.view
  outgoing = environment.tabContainer?.accessibilityChildren()?.first as? NSView
