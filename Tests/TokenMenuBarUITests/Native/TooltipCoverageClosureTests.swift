...
struct TooltipCoverageClosureTests {
  @Test @MainActor func defaultCursorContextDoesNotPresentWithoutAnAppWindow() async throws {
    prepareTestApp()
    try #require(
      NSApp.orderedWindows.allSatisfy { !$0.isVisible },
      "Visible test windows: \(NSApp.orderedWindows.filter(\.isVisible).map { "\(type(of: $0)) \($0.title) \($0.frame)" })"
    )
