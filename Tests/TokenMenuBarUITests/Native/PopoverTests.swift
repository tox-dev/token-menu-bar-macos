...
@Test(arguments: [-22.0, 22.0]) @MainActor
func popoverKeepsItsTopWhenTheBackingWindowMovesWithoutResizing(offset: Double) throws {
  ...
  window.setFrameOrigin(CGPoint(x: initial.minX, y: initial.minY + offset))
  #expect(window.frame == initial)
}

@Test(arguments: [false, true]) @MainActor func popoverRecoversOnlyAnOffscreenVerificationAnchor(
  offscreen: Bool
) throws {
  prepareTestApp()
  let log = makeLog()
  ...
  defer { anchorWindow.orderOut(nil) }
  controller.show(relativeTo: anchor, anchorFrame: anchorWindow.frame, visibleFrame: visibleFrame, screenFrame: screen)
  defer { controller.close() }
