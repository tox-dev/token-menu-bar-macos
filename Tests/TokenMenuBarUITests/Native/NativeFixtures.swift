...
@MainActor
func loadNativeAccessibility() async throws {
...
@MainActor
func pressNativeElement(in root: AnyObject, matching matches: (AnyObject) -> Bool) -> Bool {
  var visited: Set<ObjectIdentifier> = []
  func press(_ element: AnyObject) -> Bool {
    guard visited.insert(ObjectIdentifier(element)).inserted else { return false }
    if matches(element), element.accessibilityPerformPress?() == true { return true }
    if (element.accessibilityChildren?() ?? []).contains(where: { press($0 as AnyObject) }) { return true }
    return (element as? NSView)?.subviews.contains(where: { press($0) }) ?? false
  }
  return press(root)
}
