...
@Test @MainActor func statusPreviewRendersCellsWithoutWindowKeys() {
  ...
  #expect(
    inkFraction(StatusPreview(model: model, highlightedKey: .constant(nil), select: { _ in }), width: 180, height: 48)
      > 0)
}

@Test @MainActor func statusPreviewWrapsCellsAtNarrowWidths() {
  let model = statusModel()
  let preview = StatusPreview(model: model, highlightedKey: .constant(nil), select: { _ in })
  let wide = fittingSize(preview, width: 700)
  let narrow = fittingSize(preview, width: 180)
...
@Test @MainActor func providerCardRefreshesItsProvider() throws {
  let environment = try makeEnvironment()
  var refreshed: [ProviderID] = []
  let card = ProviderCardView(
    card: try #require(environment.cards.first), environment: environment,
    onRefreshProvider: { refreshed.append($0) })

  card.refresh()

  #expect(refreshed == [card.card.provider])
}
...
@Test @MainActor func settingsDeferredSectionsBecomeAccessible() async throws {
  let environment = try makeEnvironment()
  let hosting = host(SettingsTab(environment: environment), width: 880, height: 1_600)
  #expect(
    await waitUntil {
      hosting.layoutSubtreeIfNeeded()
      hosting.displayIfNeeded()
      return modelFilterField(in: hosting) != nil
    })
