...
@Test @MainActor func rootViewHostsEveryTab() async throws {
  ...
  #expect(environment.log.text.contains("tab.content-selected tab=History durationMs="))
  #expect(environment.log.text.contains("tab.viewport-applied tab=History durationMs="))
}

@Test(arguments: [CGSize(width: 480, height: 360), CGSize(width: 880, height: 720)]) @MainActor
func rootUsesTheProposedViewport(size: CGSize) throws {
  prepareTestApp()
  let controller = NSHostingController(rootView: RootView(environment: try makeEnvironment(), onMeasure: { _ in }))

  #expect(controller.sizeThatFits(in: size) == size)
}

@Test @MainActor func rootIdealSizeDoesNotDependOnItsCachedTabs() throws {
  prepareTestApp()
  let controller = NSHostingController(
    rootView: RootView(environment: try makeEnvironment(), onMeasure: { _ in }).fixedSize())

  #expect(
    controller.sizeThatFits(in: CGSize(width: 880, height: 720))
      == CGSize(width: PopoverGeometry.stableTabWidth, height: PopoverGeometry.minimumHeight))
}

@Test @MainActor func rootTabResizeKeepsTheOutgoingHostsCachedFrame() throws {
  prepareTestApp()
  let environment = try makeEnvironment()
  var outgoing: NSView?
  var outgoingFrameDuringResize: CGRect?
  let root = RootView(
    environment: environment, onMeasure: { _ in },
    onTabChange: { _ in
      environment.tabContainer?.setFrameSize(CGSize(width: 880, height: 500))
      environment.tabContainer?.layoutSubtreeIfNeeded()
      outgoingFrameDuringResize = outgoing?.frame
    })
  let view = root.makeNativeView()
  view.frame = CGRect(x: 0, y: 0, width: 880, height: 900)
  view.layoutSubtreeIfNeeded()
  outgoing = environment.tabContainer?.subviews.first { !$0.isHidden }
  let cachedFrame = try #require(outgoing).frame

  root.select(.settings)

  #expect(outgoingFrameDuringResize == cachedFrame)
}

@Test @MainActor func nativeRootExposesEachTabAsAnAccessibleSegment() throws {
  prepareTestApp()
  let view = RootView(environment: try makeEnvironment(), onMeasure: { _ in }).makeNativeView()
  view.frame = CGRect(x: 0, y: 0, width: 880, height: 900)
  view.layoutSubtreeIfNeeded()
  let controls: [NSSegmentedControl] = findViews(in: view)
  let control = try #require(controls.first { $0.accessibilityLabel() == "Popover tabs" })
  let segments = accessibilityDescendants(in: control).filter { $0.accessibilityRole() == .radioButton }

  #expect(segments.map { $0.accessibilityLabel() } == PopoverTab.allCases.map(\.rawValue))
}

@Test(arguments: PopoverTab.allCases, [false, true]) @MainActor
func nativeRootAccessibilityHitTestingFindsEachTab(tab: PopoverTab, throughWindow: Bool) throws {
  prepareTestApp()
  let view = RootView(environment: try makeEnvironment(), onMeasure: { _ in }).makeNativeView()
  let window = NonpresentingWindow(content: view)
  defer {
  ...
  let control = try #require(controls.first { $0.accessibilityLabel() == "Popover tabs" })
  let segments = accessibilityDescendants(in: control).filter { $0.accessibilityRole() == .radioButton }
  let segment = try #require(segments.first { $0.accessibilityLabel() == tab.rawValue })
  let frame = segment.accessibilityFrame()
  let point = CGPoint(x: frame.midX, y: frame.midY)
  let target = try #require(
    (throughWindow ? window.accessibilityHitTest(point) : view.accessibilityHitTest(point))
      as? any NSAccessibilityProtocol)

  #expect(target.accessibilityLabel() == tab.rawValue)
  #expect(target.accessibilityRole() == .radioButton)
  #expect(!window.isVisible)
}

@Test @MainActor func nativeRootPresentsProgrammaticTabChanges() async throws {
  prepareTestApp()
  let environment = try makeEnvironment()
  environment.log.debugEnabled = true
  ...
  environment.beginTabTransition(to: .settings)
  environment.settings.lastTab = .settings
  view.layoutSubtreeIfNeeded()

  #expect(await waitUntil { environment.log.text.contains("tab.presented to=Settings") })
  let fields: [NSTextField] = findViews(in: view)
  #expect(fields.contains { $0.placeholderString == "Filter models…" && !$0.isHiddenOrHasHiddenAncestor })
}

@Test @MainActor func tabHitDiagnosticsIdentifyTheNativeSegmentsWithoutPresentation() throws {
  prepareTestApp()
  let environment = try makeEnvironment()
  environment.log.debugEnabled = true
  let view = RootView(environment: environment, onMeasure: { _ in }).makeNativeView()
  ...
  view.logTabHitTesting(to: environment.log)

  for tab in PopoverTab.allCases {
    #expect(environment.log.text.contains("windowHit=\(tab.rawValue):AXRadioButton"))
  }
}

@Test @MainActor func tabHitDiagnosticsIgnoreContentWithoutTabControls() {
  prepareTestApp()
  let log = makeLog()
  ...
  view.logTabHitTesting(to: log)

  #expect(log.text.isEmpty)
}

@Test(arguments: [false, true]) @MainActor
func rootTabsKeepOneFrameAcrossSwitches(native: Bool) async throws {
  prepareTestApp()
  let environment = try makeEnvironment()
  let root = RootView(environment: environment, onMeasure: { _ in }, onTabChange: { _ in })
  let hosting: NSView = native ? root.makeNativeView() : host(root, width: 880, height: 900)
  hosting.frame = CGRect(x: 0, y: 0, width: 880, height: 900)
  await mainActorTurn()
  await mainActorTurn()
  ...
  let frame = tabControl.convert(tabControl.bounds, to: hosting)

  for tab in PopoverTab.allCases + PopoverTab.allCases {
    root.select(tab)
    hosting.layoutSubtreeIfNeeded()
    #expect(tabControl.convert(tabControl.bounds, to: hosting) == frame)
  }
}
...
@Test @MainActor func usageTabRendersCardsAndEmptyStates() throws {
  ...
  let card = ProviderCardView(card: try #require(stale.cards.first), environment: stale, onRefreshProvider: { _ in })
...
@Test @MainActor func usageDemoKeepsSetupWithoutAnotherModeControl() throws {
  ...
  #expect(buttons.map(\.label) == [Text(verbatim: "Open Providers")])
...
@Test @MainActor func emptyUsageOpensProviderSettings() throws {
  let environment = try makeEnvironment(populate: false)
  var providers: [ProviderID?] = []
  environment.actions.showProviders = { providers.append($0) }
  let buttons = findNativeTextButtons(in: UsageTab(environment: environment).body)
  #expect(buttons.count == 1)
  try #require(buttons.first).action()
  #expect(providers.count == 1 && providers[0] == nil)
}
...
@Test @MainActor func providerRecoveryCardRoutesToSettings() throws {
  ...
  let card = ProviderCardView(
    card: try #require(environment.cards.first), environment: environment, onRefreshProvider: { _ in })
  card.showProviders()
...
@Test(arguments: [180.0, 280.0]) @MainActor
func usageIdentityChipWrapsLongLabelsAndCopies(width: Double) throws {
  let title = "Max 20x plan for a large multi-team organisation with extended usage and additional credits"
  var copied: [String] = []
  let hosting = host(
    UsageIdentityChip(chip: Chip(text: title), provider: .claude, onCopy: { copied.append($0) }),
    width: width, height: 180)
  let buttons: [NSButton] = findViews(in: hosting)
  let control = try #require(buttons.first { $0.title == title })
  let textBounds = control.cell!.titleRect(forBounds: control.bounds)
  let required = (title as NSString).boundingRect(
    with: CGSize(width: textBounds.width, height: .greatestFiniteMagnitude),
    options: [.usesLineFragmentOrigin, .usesFontLeading], attributes: [.font: control.font!])
  #expect(required.height > control.font!.pointSize * 2)
  #expect(textBounds.height >= ceil(required.height), "Text bounds: \(textBounds), required: \(required)")
  #expect(hosting.bounds.contains(hosting.convert(control.bounds, from: control)))
  control.performClick(nil)
  #expect(copied == [title])
}

@Test @MainActor func tooltipAnchorsDoNotCaptureMouseClicks() throws {
  let hosting = host(
    Button("Refresh", action: {}).richHelp(TooltipContent(title: "Refresh", body: "Fetches current limits.")),
    width: 200, height: 40)
  let anchors: [TooltipTrackingView] = findViews(in: hosting)
  let anchor = try #require(anchors.first)
  #expect(!anchor.frame.isEmpty)
  #expect(anchor.hitTest(CGPoint(x: anchor.frame.midX, y: anchor.frame.midY)) == nil)
}

@Test @MainActor func usageIdentityChipNamesItsNativeCopyButton() throws {
  let title = "Workspace billing account"
  let hosting = host(UsageIdentityChip(chip: Chip(text: title), provider: .claude, onCopy: { _ in }))
  let buttons: [NSButton] = findViews(in: hosting)
  let button = try #require(buttons.first { $0.title == title })
  #expect(button.accessibilityLabel() == title)
}

@Test @MainActor func usageIdentityChipsDistinguishRepeatedPlanNames() {
  let hosting = host(
    VStack {
      ForEach([ProviderID.codex, .gemini, .cursor], id: \.self) { provider in
        UsageIdentityChip(chip: Chip(text: "Pro"), provider: provider, onCopy: { _ in })
      }
    })
  let buttons: [NSButton] = findViews(in: hosting)
  #expect(
    Set(buttons.filter { $0.title == "Pro" }.compactMap { $0.accessibilityIdentifier() })
      == ["provider-chip-codex-Pro", "provider-chip-gemini-Pro", "provider-chip-cursor-Pro"])
}

@Test(arguments: [180.0, 280.0]) @MainActor
func usageIdentityChipTracksItsNativeButton(width: Double) throws {
  var copied: [String] = []
  let chip = UsageIdentityChip(chip: Chip(text: "Pro"), provider: .codex, onCopy: { copied.append($0) })
  let hosting = host(chip, width: width, height: 80)
  let buttons: [NSButton] = findViews(in: hosting)
  let button = try #require(buttons.first { $0.accessibilityIdentifier() == "provider-chip-codex-Pro" })
  let anchors: [TooltipTrackingView] = findViews(in: button)
  let anchor = try #require(anchors.first)
  #expect(!button.bounds.isEmpty)
  #expect(anchor.frame == button.bounds)
  #expect(anchor.tooltipContent == chip.primaryHelp)
  #expect(anchor.trackingAreas.count == 1)
  #expect(anchor.hitTest(CGPoint(x: anchor.frame.midX, y: anchor.frame.midY)) == nil)
...
@Test @MainActor func usageEnvironmentStoresPresentationAndAdvancesLeafClock() throws {
  ...
  #expect(try #require(environment.nextUsageDeadline()).timeIntervalSince(fixedNow) < 0.001)
...
func usageEnvironmentKeepsOpenPanelPresentationCurrentAcrossTabs(tab: PopoverTab) async throws {
  let environment = try makeEnvironment()
  environment.state.setRefreshing(false, at: nil)
  environment.state.popoverVisible = true
  environment.settings.lastTab = tab
...
@Test(arguments: [false, true]) @MainActor
func inactiveUsageCoalescesRefreshUpdatesUntilTheBatchFinishes(panelOpen: Bool) async throws {
  let environment = try makeEnvironment()
  environment.settings.lastTab = .settings
  environment.state.popoverVisible = panelOpen
  let previous = environment.cards
  environment.state.setRefreshing(true, at: nil)
  environment.state.update(.codex) { $0.snapshot = sampleSnapshot(.codex, percent: 12) }
  await mainActorTurn()
  await mainActorTurn()
  #expect(environment.cards == previous)

  environment.state.setRefreshing(false, at: fixedNow)
  if !panelOpen {
    await mainActorTurn()
    await mainActorTurn()
    #expect(environment.cards == previous)
...
@MainActor private func findNativeTextButtons(in value: Any, depth: Int = 0) -> [NativeActionButton<Text>] {
...
@Test @MainActor func settingsTabRendersAndMutates() throws {
  ...
  #expect(inkFraction(SettingsTab(environment: environment, mountsIncrementally: false), width: 520, height: 1200) > 0)
  ...
  #expect(
    inkFraction(
      StatusPreview(model: statusModel(), highlightedKey: .constant(nil), select: { _ in }), width: 300, height: 40) > 0
  )
...
@Test @MainActor func tabPickerIsNamedForVoiceOver() {
  ...
  #expect(control?.accessibilityIdentifier() == "popover-tabs")
...
@Test @MainActor func logTextViewUpdatesItsHeight() {
  let entries = [LogEntry(timestamp: fixedNow, level: .info, message: "one")]
  let hosting = host(LogTextView(entries: entries, height: 100), width: 300, height: 160)
  let scroll: NSScrollView = hosting.subviews.compactMap { $0 as? NSScrollView }.first ?? findView(hosting)!

  hosting.rootView = LogTextView(entries: entries, height: 140)
  hosting.layoutSubtreeIfNeeded()

  #expect(
    scroll.constraints.first { $0.firstAttribute == .height && $0.isActive }?.constant == 140)
}

@Test @MainActor func logTextViewUpdatesItsBorder() {
  let entries = [LogEntry(timestamp: fixedNow, level: .info, message: "one")]
  let hosting = host(LogTextView(entries: entries, height: 100), width: 300, height: 120)
  let scroll: NSScrollView = hosting.subviews.compactMap { $0 as? NSScrollView }.first ?? findView(hosting)!

  hosting.rootView = LogTextView(entries: entries, height: 100, bordered: false)
  hosting.layoutSubtreeIfNeeded()
...
@Test @MainActor func windowRowsOutlineBarsAndDeepenColoursWithIncreasedContrast() throws {
  let card = UsagePresenter.card(
    provider: .claude, state: ProviderState(snapshot: sampleSnapshot(.claude), availability: .current), samples: [:],
    now: fixedNow)
  let row = card.rows[0]
  let standard = WindowRowView(row: row, now: fixedNow)
  let contrast = WindowRowView(row: row, now: fixedNow, increaseContrast: true)
  #expect(!standard.increaseContrast)
  #expect(contrast.increaseContrast)
  #expect(contrast.usageBarColor != standard.usageBarColor)
  let deepened = UsageColor.color(pace: row.pace.status, percent: row.window.usedPercent, contrast: .increased)
  #expect(contrast.usageBarColor == Color(deepened))
  #expect(inkFraction(contrast, width: 820, height: 50) > 0)
  #expect(inkFraction(contrast.environment(\.dynamicTypeSize, .accessibility1), width: 548, height: 150) > 0)
  let environment = try makeEnvironment()
  #expect(!WindowRowView(row: row, environment: environment).increaseContrast)
  environment.increaseContrast = true
  #expect(WindowRowView(row: row, environment: environment).increaseContrast)
