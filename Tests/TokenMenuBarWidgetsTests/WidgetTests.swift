...
@Test func timelineProviderReadsStoreOrPlaceholder() throws {
  ...
  #expect(UsageTimelineProvider.refreshInterval == WidgetSnapshot.refreshInterval)
  #expect(
...
@Test @MainActor func widgetRowsOutlineBarsAndDeepenColoursWithIncreasedContrast() {
  let row = WidgetSnapshot.placeholder.rows[0]
  let standard = WidgetRowView(row: row, now: fixedNow, compact: false)
  let contrast = WidgetRowView(row: row, now: fixedNow, compact: false, increaseContrast: true)
  #expect(contrast.color != standard.color)
  #expect(inkFraction(contrast, width: 300, height: 60) > 0)
  let widget = UsageWidgetView(entry: populatedEntry, family: .systemMedium, increaseContrast: true)
  #expect(inkFraction(widget, width: 300, height: 300) > 0)
}
...
@Test @MainActor func widgetRowsRenderUnavailableAndElapsedResetTimes() {
  let key = WindowKey(provider: .claude, windowID: "session")
  let unavailable = WidgetRow(
    key: key, providerName: "Claude", label: "Session", usedPercent: 20, resetsAt: nil)
  let elapsed = WidgetRow(
    key: key, providerName: "Claude", label: "Session", usedPercent: 20,
    resetsAt: fixedNow.addingTimeInterval(-60))

  #expect(inkFraction(WidgetRowView(row: unavailable, now: fixedNow, compact: false), width: 300, height: 60) > 0)
  #expect(inkFraction(WidgetRowView(row: elapsed, now: fixedNow, compact: false), width: 300, height: 60) > 0)
}
...
private func host<Content: View>(_ view: Content, width: CGFloat, height: CGFloat) -> NSHostingView<Content> {
    ...
    precondition(
      application.activationPolicy() == .prohibited || application.setActivationPolicy(.prohibited),
      "Local tests must prohibit presentation")
  #endif
  ...
  #if NONPRESENTING_TESTS
    precondition(application.windows.allSatisfy { !$0.isVisible }, "Local tests must not order windows on screen")
  #endif
  return hosting
}

@Test @MainActor func widgetRowsPrintTheShareTheSnapshotAsksFor() {
  let row = WidgetSnapshot.placeholder.rows[0]
  let remaining = WidgetRowView(row: row, now: fixedNow, compact: false, display: .remaining)

  #expect(row.percentText(display: .remaining) == "64%")
  #expect(row.percentText(display: .used) == "36%")
  #expect(remaining.display == .remaining)
  #expect(WidgetRowView(row: row, now: fixedNow, compact: false).display == .used)
  #expect(inkFraction(remaining, width: 300, height: 60) > 0)
  let entry = UsageEntry(
    date: fixedNow,
    snapshot: WidgetSnapshot(
      rows: WidgetSnapshot.placeholder.rows, attention: false, updatedAt: fixedNow, display: .remaining))
  #expect(inkFraction(UsageWidgetView(entry: entry, family: .systemMedium), width: 300, height: 300) > 0)
}
