...
`just test` and `swift test` run nonpresenting tests with generated data. Their render fixtures have no windows. CI also
requires the native package suite, full coverage and scripted application checks. `just test-native`, `just coverage`
and `just ui` require a GitHub-hosted desktop; do not bypass their guards on a developer Mac. See
...
- CI runs `Scripts/coverage.sh`, which fails when a line in Core or UI never runs. Its `glue` array lists the files that
  ...
  caps each file at 40 lines, so logic cannot accumulate where no test reaches.
...
and removes its defaults and support files after each test. CI runs on GitHub-hosted macOS 14, 15, 26, and 27 desktops
and checks the runtime version before testing. Releases also require the macOS 27 application checks on a hosted
desktop. An `xcode-27` runner with a macOS 26 image fails the runtime check.
